...
setup_traps
parse_common_flags "$@"
# Credentials must not become xtrace output, even with --debug or bash -x.
...
log_step "Cursor Agent Mail integration (user scope)"
_print "Cursor config: ${USER_CONFIG}"
...
if [[ -L "$USER_CONFIG" || ( -e "$USER_CONFIG" && ! -f "$USER_CONFIG" ) ]]; then
  log_err "Cursor user MCP target must be a regular, non-symlink file: ${USER_CONFIG}"
  exit 1
fi
_CONFIG_PARENT=$(dirname "$USER_CONFIG")
_PROBE="$_CONFIG_PARENT"
while [[ ! -e "$_PROBE" ]]; do
...
if [[ -L "$_PROBE" ]] \
  ...
  log_err "Cursor user MCP config must live outside every Git worktree: ${USER_CONFIG}"
  exit 1
fi
...
if [[ -f "$USER_CONFIG" ]]; then
  if ! jq -e 'type == "object" and ((.mcpServers // {}) | type == "object")' \
    ...
    log_err "Existing Cursor MCP config is not a mergeable JSON object: ${USER_CONFIG}"
    exit 1
  fi
...
_MERGED=$(AGENT_MAIL_JQ_AUTHORIZATION="Bearer ${_TOKEN}" jq \
  ...
  ' <<<"$_EXISTING") || {
  log_err "Could not merge Cursor user MCP configuration."
  exit 1
}
...
_SLOT="$(integration_slot "${AGENT_MAIL_CURSOR_SLOT:-1}")" || exit 1
_AGENT="$(integration_agent_name cursor "$_SLOT")" || exit 1
...
if ! am_call ensure_project "$_ENSURE_ARGS" >/dev/null 2>&1; then
  log_warn "Cursor config installed; server bootstrap is deferred because the endpoint is unavailable."
  exit 0
fi
if [[ -n "$_REG_TOKEN" ]]; then
  _REGISTER_ARGS=$(AGENT_MAIL_JQ_REGISTRATION_TOKEN="$_REG_TOKEN" jq -nc \
    --arg key "$PROJECT_KEY" --arg name "$_AGENT" \
    '{project_key:$key,program:"cursor",model:"cursor",name:$name,
...
_REGISTER_DATA="$(am_call register_agent "$_REGISTER_ARGS")" || {
  log_warn "Cursor config installed; durable-agent bootstrap will retry on the next run."
  exit 0
}
...
if [[ "$_REGISTERED_AGENT" != "$_AGENT" ]]; then
  log_err "Server did not confirm the requested durable Cursor identity."
  exit 1
fi
if [[ -n "$_REG_TOKEN" && -n "$_RETURNED_TOKEN" && "$_REG_TOKEN" != "$_RETURNED_TOKEN" ]]; then
  log_err "Server returned a different credential for the existing Cursor identity."
  exit 1
fi
if [[ -z "$_REG_TOKEN" ]]; then
  if [[ -z "$_RETURNED_TOKEN" ]] || ! am_cred_put "$PROJECT_KEY" "$_AGENT" "$_RETURNED_TOKEN"; then
    log_err "Cursor identity was registered but its credential could not be persisted safely."
    exit 1
  fi
fi

log_ok "Cursor user integration complete."
