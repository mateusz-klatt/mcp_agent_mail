...
USER_CONFIG="${HOME}/.codeium/windsurf/mcp_config.json"
SHARED_ENV_FILE="${AGENT_MAIL_ENV_FILE:-${HOME}/.agent-mail.env}"
AGENT_MAIL_ENV_FILE="$SHARED_ENV_FILE"

log_step "Windsurf Agent Mail integration (user scope)"
_print "Windsurf config: ${USER_CONFIG}"
...
if [[ -L "$USER_CONFIG" || ( -e "$USER_CONFIG" && ! -f "$USER_CONFIG" ) ]]; then
  log_err "Windsurf user MCP target must be a regular, non-symlink file: ${USER_CONFIG}"
  exit 1
fi
...
if [[ -L "$_PROBE" ]] || path_is_inside_git "$_PROBE"; then
  log_err "Windsurf user MCP config must live outside every Git worktree: ${USER_CONFIG}"
  exit 1
fi
...
if [[ -f "$USER_CONFIG" ]]; then
  if ! jq -e 'type == "object" and ((.mcpServers // {}) | type == "object")' \
    ...
    log_err "Existing Windsurf MCP config is not a mergeable JSON object: ${USER_CONFIG}"
    exit 1
  fi
...
_MERGED=$(AGENT_MAIL_JQ_AUTHORIZATION="Bearer ${_TOKEN}" jq \
  ...
  ' <<<"$_EXISTING") || {
  log_err "Could not merge Windsurf user MCP configuration."
  exit 1
}
...
_REGISTER_DATA="$(am_call register_agent "$_REGISTER_ARGS")" || {
  log_warn "Windsurf config installed; durable-agent bootstrap will retry on the next run."
  exit 0
}
...
if [[ "$_REGISTERED_AGENT" != "$_AGENT" ]]; then
  log_err "Server did not confirm the requested durable Windsurf identity."
  exit 1
fi
if [[ -n "$_REG_TOKEN" && -n "$_RETURNED_TOKEN" && "$_REG_TOKEN" != "$_RETURNED_TOKEN" ]]; then
  log_err "Server returned a different credential for the existing Windsurf identity."
  exit 1
fi
if [[ -z "$_REG_TOKEN" ]]; then
  if [[ -z "$_RETURNED_TOKEN" ]] || ! am_cred_put "$PROJECT_KEY" "$_AGENT" "$_RETURNED_TOKEN"; then
    log_err "Windsurf identity was registered but its credential could not be persisted safely."
    exit 1
  fi
fi

log_ok "Windsurf user integration complete."
